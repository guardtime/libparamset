...
/**
 * Adds tasks to the task set. Task is composed from
 ...
 * - \c forbidden - Any parameter from this group must <b>NOT</b> be set.
...
/**
 * Performs analysis on the #TASK_SET with parameters from #PARAM_SET. After analyzing
 ...
 * #TASK_SET stands out as the possible target and give user some hints how to fix it.
 * If it is not possible to assume what task the user is trying to accomplish, shows
 ...
 *
 * The analysis and error handling is based on the task \c consistency value. \c Consistency
 * \c 1.0 means that it is possible to perform the task with given parameters and
 * \b less than \c 1.0 means it is not. The \c consistency of a task is calculated
 ...
 * forbidden parameter set. The \c consistency of a task is calculated as follows:
 *
 * \code{.txt}
 ...
 * \param sensitivity	Analysis sensitivity.
...
/**
 * Extracts one single consistent task, the one that user wants to execute, from the #TASK_SET.
 * Before this #TASK_SET_analyzeConsistency must be called.
 * \param task_set		#TASK_SET object.
...
/**
 * Extracts the most consistent task from the #TASK_SET which may have more than one consistent task in it.
 ...
 * \c consistency higher than any other task by the value of \c diff.
 * \param task_set		#TASK_SET object.
 * \param diff			Task \c consistency difference.
 * \param ID			Output parameter for task \c ID.
 * \return #PST_OK if successful, error code otherwise.
 */
int TASK_SET_isOneFromSetTheTarget(TASK_SET *task_set, double diff, int *ID);

/**
 * Removes all ignored parameters (see #TASK_SET_add) from the
 ...
 * \c consistency value starting from the highest (see #TASK_SET_analyzeConsistency).
 * \param	task_set		#TASK_SET object.
...
/**
 * Generates suggestion message from pre-analyzed
 * #TASK_SET to help user figure out how to fix the task with \c ID
 * (see #TASK_SET_analyzeConsistency; see #TASK_SET_isOneFromSetTheTarget to get \c ID of the task that user is trying
 * to accomplish).
 * \param	task_set		#TASK_SET object.
 * \param	set				#PARAM_SET object.
 * \param	ID				Tasks \c ID.
...
/**
 * Gets the #PARAM_SET object used to analyze task's \c consistency
